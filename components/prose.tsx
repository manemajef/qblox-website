...
export function Prose({
  ...
  return <div className={cn(className, "max-w-2xl p-4")}>{children}</div>;
}
...
export function Title({ className = "", children }: PropsType) {
  return (
    <h1
      className={cn(
        "text-5xl sm:text-6xl lg:text-8xl font-bold text-foreground/85 tracking-tight leading-tight my-4",
        className
      )}
    >
      {children}
...
export function H1({ className = "", children }: PropsType) {
  return (
    <h1
      className={cn(
        className,
        "text-5xl sm:text-6xl lg:text-7xl font-semibold leading-tight tracking-tight text-foreground/85"
      )}
    >
      {children}
    </h1>
...
export function H4({ className = "", children }: PropsType) {
  return (
    <h4
      className={cn(
        "text-2xl sm:text-3xl font-semibold leading-tight tracking-tight text-foreground/85 mt-6 mb-3",
        className
      )}
    >
      {children}
...
export function P({ className = "", children }: PropsType) {
  return (
    <p
      className={cn(
        "text-lg sm:text-xl leading-relaxed text-foreground/80 mb-6",
        className
      )}
    >
      {children}
...
export function Lead({ className = "", children }: PropsType) {
  return (
    <p
      className={cn(
        "text-xl sm:text-2xl text-muted-foreground mb-8 font-light leading-relaxed",
        className
      )}
    >
      {children}
...
export function Ul({ className = "", children }: PropsType) {
  return (
    <ul
      className={cn(
        "list-disc list-outside ml-6 space-y-2 mb-6 marker:text-muted-foreground",
        className
      )}
    >
      {children}
...
export function Quote({ className = "", children }: PropsType) {
  return (
    <blockquote
      className={cn(
        "border-l-4 border-primary pl-6 italic text-xl sm:text-2xl text-muted-foreground my-8 py-2",
        className
      )}
    >
      {children}
...
export function Link({
  ...
  href,
  children,
}: PropsType & { href?: string }) {
  return (
    <a
      href={href}
      className={cn(
        "font-medium text-primary underline underline-offset-4 hover:text-primary/80 transition-colors cursor-pointer",
        className
      )}
    >
      {children}
  ...
  return (
    <div className="min-h-screen bg-background">
      <Prose className="mx-auto py-24">
        ...
        <Lead>
          Discover how simple cubes can transform into complex architectural
          masterpieces through creativity, engineering, and a bit of
          imagination.
        </Lead>

        <H1>Getting Started</H1>
        <P>
          Qblox is more than just a toy; it's a{" "}
          <Strong>tool for imagination</Strong>. With our unique interlocking
          system, you can build in <Link href="#">three dimensions</Link>,
          defying gravity and expectation.
        </P>

        <H2>Core Concepts</H2>
        <P>
          Understanding the basics is crucial. Here is what you need to know to
          start your journey:
        </P>
        <Ul>
        ...
        <Quote>"The only limit is your imagination. And maybe gravity."</Quote>

        <H3>Advanced Techniques</H3>
        <P>Once you've mastered the basics, try these advanced moves:</P>

        <H4>Cantilevers</H4>
        <P>
          Building out into space requires balance. Ensure your base is heavy
          enough to support the extension.
        </P>
